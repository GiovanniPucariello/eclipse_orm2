...
import net.orm.diagram.edit.commands.RoleCreateCommand;
import net.orm.diagram.edit.commands.SubTypeCreateCommand;
import net.orm.diagram.edit.commands.SubTypeReorientCommand;
...
public class PredicateItemSemanticEditPolicy extends
	...
	public PredicateItemSemanticEditPolicy() {
		super(OrmElementTypes.Predicate_2001);
	}
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (OrmElementTypes.Role_3001 == req.getElementType()) {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (OrmVisualIDRegistry.getVisualID(incomingLink) == SubTypeEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
		...
		for (Iterator it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (OrmVisualIDRegistry.getVisualID(outgoingLink) == SubTypeEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
						outgoingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
